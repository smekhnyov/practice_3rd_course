...
SELECT 
    P.ProjectName,
    PM.LastName || ' ' || SUBSTRING(PM.FirstName, 1, 1) || '. ' || SUBSTRING(PM.MiddleName, 1, 1) || '.' AS ProjectManager,
    D.DateDiscovered,
    D.DefectName,
    D.ShortDescription,
    D.DetailedDescription,
    ED.LastName || ' ' || ED.FirstName || ' ' || ED.MiddleName,
    D.PlannedFixTime,
    S.StatusName,
    DSH.DateChanged,
    PR.PriorityName ,
...
JOIN 
    Employees PM ON EP.EmployeeID = PM.EmployeeID AND PM.PositionID = 3
JOIN 
    Employees ED ON D.DiscoveredBy = ED.EmployeeID
INNER JOIN 
	LastStatusChange LSC ON D.DefectID = LSC.DefectID
INNER JOIN 
	DefectStatusHistory DSH ON LSC.DefectID = DSH.DefectID AND LSC.LastChangeDate = DSH.DateChanged
JOIN 
    Statuses S ON DSH.StatusID = S.StatusID
JOIN 
    Priorities PR ON D.PriorityID = PR.PriorityID
LEFT JOIN 
    Comments C ON D.DefectID = C.DefectID
GROUP BY 
    P.ProjectName,
    PM.LastName, PM.FirstName, PM.MiddleName, 
    D.DateDiscovered, 
    D.DefectName, 
    D.ShortDescription, 
    D.DetailedDescription,
...
SELECT
    D.DefectName,
    STRING_AGG(S.StatusName, ', ') AS AllStatuses
FROM Defects AS D
LEFT JOIN DefectStatusHistory AS DSH
    ON D.DefectID = DSH.DefectID
LEFT JOIN Statuses AS S
    ON DSH.StatusID = S.StatusID
GROUP BY
    D.DefectID, D.DefectName
ORDER BY
    D.DefectID;
...
-- За ошибку перехода статуса считается, когда статус NULL, когда статус никогда не был в статусе Open и когда статус перешел из Closed в любой другой
WITH LatestStatus AS (
    SELECT 
        D.DefectID,
        MAX(DSH.DateChanged) AS LastChangeDate
    FROM Defects D
...
StatusCheck AS (
    SELECT 
        D.DefectID,
        MAX(CASE WHEN S.StatusName = 'Open' THEN 1 ELSE 0 END) AS HasOpenStatus,
        MAX(CASE WHEN S.StatusName = 'Closed' THEN DSH.DateChanged ELSE NULL END) AS LastClosedDate,
        MAX(CASE WHEN DSH.DateChanged = LS.LastChangeDate THEN S.StatusName ELSE NULL END) AS CurrentStatus
    FROM Defects D
    LEFT JOIN DefectStatusHistory DSH ON D.DefectID = DSH.DefectID
    LEFT JOIN Statuses S ON DSH.StatusID = S.StatusID
    LEFT JOIN LatestStatus LS ON D.DefectID = LS.DefectID
    GROUP BY D.DefectID
)
SELECT 
    D.DefectID,
    D.DefectName,
    CASE
        WHEN SC.CurrentStatus IS NULL THEN 'Status is NULL'
        WHEN SC.HasOpenStatus = 0 THEN 'Never in Open status'
        WHEN SC.LastClosedDate IS NOT NULL AND SC.CurrentStatus <> 'Closed' THEN 'Status changed after Closed'
    END AS StatusError
FROM Defects D
LEFT JOIN StatusCheck SC ON D.DefectID = SC.DefectID
WHERE SC.CurrentStatus IS NULL
   OR SC.HasOpenStatus = 0
   OR (SC.LastClosedDate IS NOT NULL AND SC.CurrentStatus <> 'Closed');
